    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs)
        If Not IsPostBack Then
            If Request.QueryString("search") IsNot Nothing Then
                txtTitre.Text = Request.QueryString("search")
            End If
            If Request.QueryString("cat") IsNot Nothing Then
                Dim catValue As String = Request.QueryString("cat")
                If ddlCat.Items.FindByValue(catValue) IsNot Nothing Then
...
<head>
    ...
    <style>
        ...
        .pagination-style a, .pagination-style span { padding: 5px 12px; border-radius: 4px; border: 1px solid #ddd; text-decoration: none; color: #78350f; }
        .pagination-style span { background-color: #b45309; color: white; }
...
<body>
    ...
    <nav class="nav-glass sticky top-0 z-50 shadow-lg">
        <div class="max-w-7xl mx-auto px-6 flex justify-center items-center h-16">
            <div class="flex space-x-12 text-sm font-bold uppercase tracking-widest">
                ...
                <a href="livres.aspx" class="text-white border-b-2 border-yellow-400 pb-1">Nos Livres</a>
    ...
    <main class="p-10 max-w-7xl mx-auto w-full">
        <form id="form1" runat="server">
            <div class="bg-white p-6 rounded-t-xl shadow-sm flex flex-wrap justify-center gap-6 items-end border-b">
                <div class="flex flex-col">
                    <label class="text-[10px] font-bold text-yellow-700 uppercase mb-1 tracking-wider">Recherche Titre</label>
                    <asp:TextBox ID="txtTitre" runat="server" CssClass="p-2 border border-gray-200 rounded-lg w-64 text-sm focus:ring-2 focus:ring-yellow-500 outline-none transition" AutoPostBack="true" placeholder="Ex: Informatique..."></asp:TextBox>
                </div>
                <div class="flex flex-col">
                    <label class="text-[10px] font-bold text-yellow-700 uppercase mb-1 tracking-wider">Catégorie</label>
                    <asp:DropDownList ID="ddlCat" runat="server" CssClass="p-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-yellow-500 transition" AutoPostBack="true">
                        <asp:ListItem Value="%">Toutes</asp:ListItem>
                        <asp:ListItem Value="Informatique">Informatique</asp:ListItem>
                        <asp:ListItem Value="Affaires">Affaires</asp:ListItem>
                        <asp:ListItem Value="Technologie">Technologie</asp:ListItem>
                    </asp:DropDownList>
                </div>
                <div class="flex flex-col">
                    <label class="text-[10px] font-bold text-yellow-700 uppercase mb-1 tracking-wider">État</label>
                    <asp:DropDownList ID="ddlType" runat="server" CssClass="p-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-yellow-500 transition" AutoPostBack="true">
                        <asp:ListItem Value="%">Tous</asp:ListItem>
                        <asp:ListItem Value="new">Neuf</asp:ListItem>
                        <asp:ListItem Value="used">Occasion</asp:ListItem>
                    </asp:DropDownList>
                </div>
            ...
            <div class="bg-white rounded-b-xl shadow-lg overflow-hidden">
                <asp:GridView ID="GridViewlivres" runat="server" AutoGenerateColumns="False" DataSourceID="livres" AllowPaging="True" PageSize="10" AllowSorting="True" CssClass="w-full text-left" GridLines="None">
                    ...
                    <PagerStyle CssClass="pagination-style py-6" />
                    <Columns>
                        <asp:BoundField DataField="Numlivre" HeaderText="Ref" ItemStyle-CssClass="p-5 text-gray-400 font-mono text-xs" />
                        <asp:BoundField DataField="titre" HeaderText="Titre du Livre" ItemStyle-CssClass="p-5 font-bold text-gray-800" />
                        <asp:BoundField DataField="auteur" HeaderText="Auteur" ItemStyle-CssClass="p-5 text-gray-600 italic" />
                        <asp:TemplateField HeaderText="Catégorie">
                            <ItemTemplate><span class="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter"><%# Eval("categorie") %></span></ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Prix">
                            <ItemTemplate><span class="text-amber-700 font-bold text-lg"><%# Eval("prix") %>€</span></ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                </asp:GridView>
            </div>

            <asp:SqlDataSource runat="server" ID="livres" 
                ConnectionString="<%$ ConnectionStrings:ConnectionString2 %>" 
                ProviderName="<%$ ConnectionStrings:ConnectionString2.ProviderName %>" 
                SelectCommand="SELECT * FROM [livres] WHERE ([titre] LIKE '%' + ? + '%') AND ([categorie] LIKE ?) AND ([type] LIKE ?)">
                <SelectParameters>
                    <asp:ControlParameter ControlID="txtTitre" Name="titre" PropertyName="Text" Type="String" DefaultValue="%" />
                    <asp:ControlParameter ControlID="ddlCat" Name="cat" PropertyName="SelectedValue" Type="String" DefaultValue="%" />
                    <asp:ControlParameter ControlID="ddlType" Name="type" PropertyName="SelectedValue" Type="String" DefaultValue="%" />
                </SelectParameters>
            </asp:SqlDataSource>
        </form>
    </main>
